...
import { OrderCardAccordionSkeleton } from "@pages/ordering/order-card-accordion-item";
import OrderCardAccordionItemSender from "@pages/sender-order-management/order-card-accordion-item-sender";
...
export default function SenderHistoryPage() {
  ...
  const { isLoading: fetchLoading, isFetching } = useQuery(
    ["fetchSenderHistory"],
    async () => await getAllSenderHistory(),
    {
      enabled: !isLoading,
      retry: false,
      staleTime: 0,
  ...
  return (
    <SenderLayout
      menuDescription={`Manage ${(user as User).firstName} ${
